//! Tests for SetLenderRequiredAttrs (contract owner only). No funds accepted.

use crate::constants::MAX_LENDER_BORROWER_REQUIRED_ATTRS;
use crate::contract::execute;
use crate::execute::set_lender_required_attrs::ASSERT_CUSTODIAN_ERR;
...
#[test]
fn set_lender_required_attrs_owner_succeeds_and_updates_state() {
    let (mut deps, env) = setup_instantiated_contract();
    let attrs = vec!["lender.kyc".to_string(), "lender.accredited".to_string()];
    execute(
        deps.as_mut(),
        env,
        message_info(&Addr::unchecked(CUSTODIAN), &[]),
        ExecuteMsg::SetLenderRequiredAttrs {
            lender_required_attrs: attrs.clone(),
        },
    )
    .expect("owner set_lender_required_attrs should succeed");
    let contract = get_contract_state_v1(deps.as_ref().storage).unwrap();
    assert_eq!(contract.lender_required_attrs, attrs);
}

#[test]
fn set_lender_required_attrs_owner_fails() {
    let (mut deps, env) = setup_instantiated_contract();
    let err = execute(
        deps.as_mut(),
        env,
...
#[test]
fn set_lender_required_attrs_non_custodian_user_fails() {
    let (mut deps, env) = setup_instantiated_contract();
    let err = execute(
        deps.as_mut(),
        env,
...
#[test]
fn set_lender_required_attrs_non_custodian_fails() {
    let (mut deps, env) = setup_instantiated_contract();
    let err = execute(
        deps.as_mut(),
        env,
...
#[test]
fn set_lender_required_attrs_fails_with_funds() {
    let (mut deps, env) = setup_instantiated_contract();
    let err = execute(
        deps.as_mut(),
        env,
...
#[test]
fn set_lender_required_attrs_empty_list_allowed() {
    let (mut deps, env) = setup_instantiated_contract();
    execute(
        deps.as_mut(),
        env,
    ...
    let contract = get_contract_state_v1(deps.as_ref().storage).unwrap();
    assert!(contract.lender_required_attrs.is_empty());
}

#[test]
fn set_lender_required_attrs_fails_when_over_max_attrs() {
    let (mut deps, env) = setup_instantiated_contract();
    let attrs: Vec<String> = (0..MAX_LENDER_BORROWER_REQUIRED_ATTRS + 1)
        .map(|i| format!("lender.attr.{i}"))
        .collect();
    let err = execute(
        deps.as_mut(),
        env,
        ...
        ContractError::IllegalArgumentError { message } => {
            assert!(
                message.contains(&MAX_LENDER_BORROWER_REQUIRED_ATTRS.to_string())
                    && message.to_lowercase().contains("lender"),
                "expected max lender attrs message, got {}",
                message
            );
        }
...
#[test]
fn set_lender_required_attrs_fails_on_invalid_wildcard_pattern() {
    let (mut deps, env) = setup_instantiated_contract();
    let err = execute(
        deps.as_mut(),
        env,
        message_info(&Addr::unchecked(CUSTODIAN), &[]),
        ExecuteMsg::SetLenderRequiredAttrs {
            lender_required_attrs: vec!["kyb.*.pb".to_string()],
        },
    )
    .unwrap_err();
    match &err {
