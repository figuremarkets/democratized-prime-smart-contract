...
| `max_liquidation_staleness_seconds` | Outer bound on last-known oracle prices for liquidation (default **1 hour**, hard cap **24 hours**). Expired prices older than this are unpriceable (valued at $0, not seizable). `0` disables last-known. Borrow/RemoveCollateral still give no credit for collateral past the *oracle* freshness window (seconds); this bound applies only to Liquidate. Do not enable **permissionless** liquidation against the 24-hour cap. |
| `liquidation_access` | **OwnerOnly** (default) or **Permissionless**. Who may call **Liquidate**. Set at instantiate or **UpdateContractConfig**. Paused still blocks Liquidate for everyone. |
...
- **UpdateContractConfig** – Contract owner only; no funds. Optional fields: margin_rate, liquidation_rate, liquidation_bonus_rate, price_oracle_address, min_lend, min_borrow, max_borrower_collateral_types, **max_liquidation_staleness_seconds** (omit field or JSON `null` = no change; `0` disables last-known; cannot exceed 24 hours), **liquidation_access** (`owner_only` / `permissionless`; omit = no change), **commit_market_id** (integer = set Provenance market id; omit field or JSON `null` = no change to this field; there is no “clear” via this message), **bad_debt_loss_allocation** (may change only when **`deficit_underlying`** is 0; repeating the current value is allowed). Only provided (non-null) fields are updated. After apply, invariants are checked: margin_rate &lt; liquidation_rate, liquidation_bonus_rate &gt; 1, bonus × margin_rate &lt; 1. **Liquidation rate may only be increased** (never decreased), so the contract owner cannot lower the LTV bar and make previously safe positions liquidatable.
