...
| `custodian` | Address authorized for contract-management operations, including **UpdateContractConfig**. |
| `liquidator` | Address authorized to call **Liquidate** when `liquidation_access` is **LiquidatorOnly**. Required at instantiate; migration backfills the current owner for legacy state without this field. |
...
| `max_liquidation_staleness_seconds` | Outer bound on last-known oracle prices for liquidation (default **1 hour**, hard cap **24 hours** under **liquidator-only**). Expired prices older than this are unpriceable (valued at $0, not seizable). `0` disables last-known. Borrow/RemoveCollateral still give no credit for collateral past the *oracle* freshness window (seconds); this bound applies only to Liquidate. **Permissionless** last-known cannot exceed **1 hour** (instantiate and `UpdateContractConfig` reject the combination). |
| `liquidation_access` | **LiquidatorOnly** (default) or **Permissionless**. Who may call **Liquidate**. Set at instantiate or **UpdateContractConfig**. Legacy on-chain JSON may still store `owner_only`; load maps that to liquidator-only and **migrate** rewrites it to `liquidator_only`. Permissionless requires last-known ≤ 1 hour, **immediate** haircut, and `deficit_underlying == 0`. Unpriceable collateral that is **load-bearing** (counting last-known of dropped feeds would make the position not liquidatable, or a dropped feed has no stored quote) still requires the owner. Dust of a dead feed does not. Paused still blocks Liquidate for everyone. |
...
**InstantiateMsg:** Contract name, description, **`repo_token`** (**`existing`** with `repo_token_cw20_contract_address`, or **`new`** with code id + name/symbol/decimals to instantiate `repo_token_cw20` in the same tx—the SubMsg uses **`democratized_prime_lib::repo_token::InstantiateMsg`**), lending denom, **rate_params** (kink model), lender/borrower attributes, price oracle, collateral limits, required **custodian** and **liquidator** addresses, **max_liquidation_staleness_seconds** (optional; default 1 hour, cap 24 hours under liquidator-only / 1 hour under permissionless), **liquidation_access** (optional; default liquidator-only; permissionless requires last-known ≤ 1 hour and immediate haircut; load-bearing unpriceable collateral still requires the owner), LTV params, **min_lend**, **min_borrow**, supported collateral assets.
...
- **UpdateContractConfig** – Contract custodian only; no funds. Optional fields: margin_rate, liquidation_rate, liquidation_bonus_rate, price_oracle_address, min_lend, min_borrow, max_borrower_collateral_types, **liquidator**, **max_liquidation_staleness_seconds** (omit field or JSON `null` = no change; `0` disables last-known; cannot exceed 24 hours under liquidator-only, or 1 hour under permissionless), **liquidation_access** (`liquidator_only` / `permissionless`; omit = no change; permissionless requires last-known ≤ 1 hour, immediate haircut, and zero deficit), **commit_market_id** (integer = set Provenance market id; omit field or JSON `null` = no change to this field; there is no “clear” via this message), **bad_debt_loss_allocation** (may change only when **`deficit_underlying`** is 0; repeating the current enum is still allowed). Only provided (non-null) fields are updated. After apply, invariants are checked: margin_rate &lt; liquidation_rate, liquidation_bonus_rate &gt; 1, bonus × margin_rate &lt; 1. **Liquidation rate may only be increased** (never decreased), so the custodian cannot lower the LTV bar and make previously safe positions liquidatable.
