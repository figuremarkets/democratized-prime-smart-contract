...
| `lender_required_attrs`, `borrower_required_attrs` | List of Provenance attribute names: sender must have **all** of these to lend (or receive Transfer) / to borrow (or add/remove collateral). Matching follows marker `--required-attributes` / `MatchAttribute` (`*.suffix` keeps the `.` in the scan suffix). Empty list = no attribute required. Updatable by the contract owner via **SetLenderRequiredAttrs** / **SetBorrowerRequiredAttrs**. |
...
| `liquidation_access` | **OwnerOnly** (default) or **Permissionless**. Who may call **Liquidate**. Set at instantiate or **UpdateContractConfig**. Permissionless requires last-known ≤ 1 hour, **immediate** haircut, and `deficit_underlying == 0`. Unpriceable collateral that is **load-bearing** (counting last-known of dropped feeds would make the position not liquidatable, or a dropped feed has no stored quote) still requires the owner. Dust of a dead feed does not. Paused still blocks Liquidate for everyone. |
...
| `bad_debt_loss_allocation` | **BadDebtLossAllocation** (default **deferred_to_deficit**): on bad-debt liquidation, either book **`deficit_underlying`** (then **EliminateDeficit** / **SocializeDeficit** — backstop/cover-from-outside; owner-only trigger) or apply an immediate pro-rata **`liquidity_index`** haircut (**immediate_liquidity_index_haircut**). Set at **instantiate** or **UpdateContractConfig** (allocation may change only when **`deficit_underlying`** is zero). **Permissionless** requires **immediate**. |
...
**InstantiateMsg:** Contract name, description, **`repo_token`** (**`existing`** with `repo_token_cw20_contract_address`, or **`new`** with code id + name/symbol/decimals to instantiate `repo_token_cw20` in the same tx—the SubMsg uses **`democratized_prime_lib::repo_token::InstantiateMsg`**), lending denom, **rate_params** (kink model), lender/borrower attributes, price oracle, collateral limits, **max_liquidation_staleness_seconds** (optional; default 1 hour, cap 24 hours under owner-only / 1 hour under permissionless), **liquidation_access** (optional; default owner-only; permissionless requires last-known ≤ 1 hour and immediate haircut; load-bearing unpriceable collateral still requires the owner), LTV params, **min_lend**, **min_borrow**, supported collateral assets.
...
- **Repay** – User sends coins; we reduce their scaled borrow and total (after accruing). Full close charges **`ceil(scaled × borrow_index)`**; excess is refunded.
...
- **Liquidate** – Auth follows **`liquidation_access`** (**owner-only** by default; **permissionless** allows any sender **unless** unpriceable collateral is load-bearing, which still requires the owner). Target borrower must be liquidatable (LTV ≥ liquidation_rate). The liquidator sends one lending coin that must reduce scaled debt (a 1-atom repay is rejected once **borrow_index > 1**); repay is **min(sent, ceil debt)**, and the repay plus seizure must leave the borrower's LTV **at or below margin_rate** — checked against the real post-state, with a full close or a full repayment exempt. They also choose **collateral_to_seize**; its **market** value must sit between 100% and the liquidation bonus multiple of the repay value, except a remainder worth **$0** (empty map, or leftover with no haircutted USD) waives the 100% floor. The bonus cap still applies, so a trivial repay that actually reduces scaled debt can only empty a dust bag. Seized collateral goes to the liquidator; debt and collateral are reduced on chain. **Bad debt path:** if the seizure would leave **no collateral** but **debt still on the books**, the contract zeroes that borrower’s debt and trims **total_scaled_borrow** correctly. Depending on **`bad_debt_loss_allocation`**, it either books **`deficit_underlying`** (**deferred**) or applies an immediate **`liquidity_index`** haircut (**immediate**). Attributes **`bad_debt_underlying`**, **`deficit_underlying`**, and **`bad_debt_loss_allocation`** describe the path. **Blocked when Paused.**
...
- **WithdrawReserve** – Contract owner only; no funds. Sends at most **`min(accrued_reserve, bank balance − lender claims)`** in the lending asset (optional recipient; else contract owner), then closes the booked reserve bucket. If the cap binds, the unbacked remainder is written off and emitted as **`unbacked_reserve_writeoff`**. Indexes are brought current first. **Not allowed if there is a deficit**—clear **`deficit_underlying`** with **EliminateDeficit** before pulling protocol fees.
...
- **UpdateContractConfig** – Contract owner only; no funds. Optional fields: margin_rate, liquidation_rate, liquidation_bonus_rate, price_oracle_address, min_lend, min_borrow, max_borrower_collateral_types, **max_liquidation_staleness_seconds** (omit field or JSON `null` = no change; `0` disables last-known; cannot exceed 24 hours under owner-only, or 1 hour under permissionless), **liquidation_access** (`owner_only` / `permissionless`; omit = no change; permissionless requires last-known ≤ 1 hour, immediate haircut, and zero deficit), **commit_market_id** (integer = set Provenance market id; omit field or JSON `null` = no change to this field; there is no “clear” via this message), **bad_debt_loss_allocation** (may change only when **`deficit_underlying`** is 0; repeating the current value is allowed). Only provided (non-null) fields are updated. After apply, invariants are checked: margin_rate &lt; liquidation_rate, liquidation_bonus_rate &gt; 1, bonus × margin_rate &lt; 1. **Liquidation rate may only be increased** (never decreased), so the contract owner cannot lower the LTV bar and make previously safe positions liquidatable.
...
- **GetBorrowerPosition(address)** – Borrower position: debt (scaled + underlying + display), `collateral` (per-asset amounts; `satisfiable` is false on holdings omitted from borrow-side USD), `unpriceable_collateral` (held denoms omitted from that USD total because the feed is missing, zero, or stale), `collateral_value_usd`, borrow-side `loan_to_value` / `health` (fresh prices, same bound as Borrow), `liquidation_ltv` / `liquidation_health` (last-known prices within `max_liquidation_staleness_seconds`, same bound as Liquidate), and `liquidation_unpriceable_collateral` (held denoms Liquidate will not seize). A stale **lending** denom still fails the query.
...
- **Displaying amounts:** Use the CW20 Balance/TokenInfo for lent supply; use `underlying_debt_display` from GetBorrowerPosition for debt. Pair with `lending_denom.name` for the unit label. A full close (Repay or Liquidate) may cost one base unit more than that floored quote; overpay, excess is refunded.
- **GetCollateralRequirements** – Optional borrower, new loan amount, and collateral asset ids. Returns required total collateral value (USD), additional value needed (when borrower set), per-asset minimum amounts with `satisfiable` so amount 0 is not confused with an unquotable feed, and `unpriceable_collateral` (held denoms omitted from that additional-needed credit). See **Section 6.2** for the full flow.
...
- **Scaling helpers** (Decimal256 with 18-decimal atomics ↔ u128):
  ...
  - **`scaled_to_underlying_borrow(scaled, borrow_index)`** – Floor/truncate. Quotes, cash, utilization, LTV.
  - **`scaled_to_underlying_borrow_ceil(scaled, borrow_index)`** – Ceil. Full close on Repay/Liquidate, and Liquidate bad-debt write-off of leftover scaled.
...
**Borrow** (`borrow.rs`)
After accruing indexes, the pool checks **available cash** the same way as for a withdraw (supplied liquidity, minus borrows, minus deficit). It then prices collateral and rejects the loan if post-borrow LTV would exceed the **margin** (position must stay healthy). LTV uses **floored** recorded debt, same as GetBorrowerPosition. On success it records scaled debt (rounded **up** so the pool never understates what it lent) and sends the lending coins.

**Repay** (`repay.rs`)
The user sends lending-denom coins. The contract accrues interest and takes a single repayment coin. Full close charges **`ceil(scaled × borrow_index)`** and zeros scaled debt (queries stay floored). Partial repay floors the scaled reduction and rejects an amount that would not reduce scaled debt. Excess over the close amount is refunded.

**AddCollateral** (`add_collateral.rs`)
The user sends one or more coins that must be supported collateral types. A **new** denom (not already in the borrower’s map) must have a **fresh** oracle price so a borrower cannot introduce an unpriceable asset. Top-ups of an already-held denom are allowed while that feed is stale, so a borrower can still defend a liquidatable position during an outage. The contract checks the borrower attribute and that the number of distinct collateral types (existing plus new) does not exceed the limit. It adds the received amounts to the borrower’s collateral balances and to the running totals per asset. The coins remain in the contract (they are the collateral).
...
- **LTV ≥ liquidation_rate:** **Liquidatable** (still requires a **Liquidate** tx). A run pays down debt and pulls collateral within the value band. A remainder worth **$0** after the seizure (empty map, or leftover with no haircutted USD) waives the 100% floor so a dust bag can be emptied even when its market value is below the repay; the bonus cap still bounds how much can be taken. If seizure leaves a remainder with **no USD value** — the map is empty, or what is left is unpriceable or truncates to $0 — but **debt remains**, the **bad-debt** path clears leftover scaled debt. This is how a position backed by a **delisted or dead-feed** asset gets resolved: it is the in-liquidation replacement for the standalone owner-only write-off entrypoint. Note the borrower **keeps** an unpriceable remainder, since unpriceable collateral cannot be seized. Supplier impact follows **`bad_debt_loss_allocation`**: **`deficit_underlying`** (**deferred**) or an immediate **`liquidity_index`** haircut (**immediate**). A **deferred** deficit can be reduced later with **EliminateDeficit** (**`bank`** from any payer or **`accrued_reserve`** by the contract owner) or **SocializeDeficit** (pro-rata haircut).

**Liquidate** (`liquidate.rs`)
Auth follows **`liquidation_access`** on contract state (**owner-only** by default; **permissionless** = any sender when the position is liquidatable even after counting unpriceable holdings at their retained last-known quotes). Paused still blocks everyone. A dropped feed with **no** stored quote, or a last-known that would pull LTV back below the liquidation rate, still requires the owner: unpriceable holdings are valued at $0 in the acted-on health check, which inflates LTV and would otherwise let a stranger seize the priced remainder of a solvent position. Dust of a dead feed does not move LTV and does not disable permissionless. The contract accrues interest and loads the borrower's debt and collateral. Liquidation uses **last-known** oracle prices when stale but still within **`max_liquidation_staleness_seconds`** of expiration (default **1 hour**, hard cap **24 hours** under owner-only so a paused feed does not freeze liquidations). Permissionless is rejected unless last-known is ≤ **1 hour**, allocation is **immediate**, and **`deficit_underlying`** is 0. The **lending denom** must have a stored price within that bound. Collateral with **no stored price**, a **zero** stored price, or last-known **older than the bound**, is valued at **$0** for LTV and **cannot be seized**. If every held collateral asset is unpriceable, Liquidate fails. Borrow and RemoveCollateral give **no credit** for missing or stale collateral as soon as the oracle marks the feed stale (typically tens of seconds) and still require a **fresh** lending-denom price — a much tighter window than liquidation last-known. The borrower must be *liquidatable*: **LTV ≥ liquidation_rate** (using those valuations).

There is **no precomputed minimum repay**. The liquidator sends lending denom in **funds** (one coin that must reduce scaled debt; sub-index amounts are rejected); actual repay = min(sent, **ceil** debt), and excess is refunded. There is no `amount` field in the message (same pattern as Repay). How much is *enough* is decided after the fact: the contract simulates the post-seizure collateral map and post-repay debt, then requires the resulting position to be **Healthy** (LTV ≤ **margin_rate**). A **full close**, a **full repayment**, and a post-state whose remaining collateral carries no USD value are exempt.

This replaced an earlier closed-form minimum, `r = (D − margin_rate·C) / (1 − liquidation_bonus_rate·margin_rate)`, which mixed units: `C` was **haircutted** collateral USD, but the seizure band bounds the seizure by **market** (un-haircutted) value, so a $1 repay only removes `haircut · bonus · $1` of health-relevant collateral. The formula overstated the requirement by roughly `1/haircut`, making it impossible to liquidate *to* margin_rate and forcing every liquidation to overshoot well past it. No corrected closed form exists, because the liquidator chooses which assets to seize and haircuts are per-asset — hence the post-state check.

The liquidator also specifies *which* collateral to seize (denom and amount per asset). Assets with a stored **non-zero** price (fresh, or stale within `max_liquidation_staleness_seconds`) may be seized at that price. Assets with **no stored price**, a **zero** stored price, or last-known **beyond the bound** cannot. The **market value** of seized collateral (`display_price_usd × amount / 10^precision`; no haircut) must lie in the band: **≥ repay value** and **≤ repay value × liquidation_bonus**, except a remainder worth **$0** waives the 100% floor. The liquidator therefore cannot take more than the bonus cap; they may pay more than the seized collateral is worth when emptying a dust position or leaving only a worthless remainder. Unpriceable balances stay on the borrower until a usable feed exists, and a mixed priced/unpriceable bag cannot full-close because the unpriceable portion cannot be seized.

The contract simulates collateral **after** the seizure before applying the band, so it knows whether the remainder is worth $0. **Normal case:** some collateral of value remains; usual partial repay and debt update; the 100% floor still applies. **Bad-debt case:** leftover collateral is worth nothing but debt would not be fully cleared—the borrower’s scaled debt is cleared, **total_scaled_borrow** drops by repay plus write-off, and reserve updates follow **`bad_debt_loss_allocation`** (deferred vs immediate; see Section 3). Cancelling all scaled debt against a remainder worth $0 — whether the map empties or an unpriceable unit is left behind — requires the ceiled payoff; sending only the floored debt is rejected so a one-unit rounding residue is not booked as bad debt. Response attributes include **`bad_debt_underlying`**, **`deficit_underlying`**, and **`bad_debt_loss_allocation`**. Collateral totals and borrower map are saved, seized coins go to the liquidator, and rate metadata is attached.
...
Contract owner only; no coins attached. Brings indexes current, then fails if **`deficit_underlying`** is still positive. Pays the lesser of **`accrued_reserve`** and actual bank cash above aggregate lender claims (optional recipient; default contract owner), then zeros the bucket. Lender claims are **`L − B`**, not the cash figure from **`reserve_totals_and_cash_u128`** (`L − B − deficit`); that is the more conservative cap and is only equivalent because deficit is already required to be zero. This solvency cap prevents any reserve-accounting defect from draining lender principal. If booked reserve exceeds that cap, the unpaid remainder is closed as well and surfaced as **`unbacked_reserve_writeoff`**.
...
Answers: “How much does this address owe?” The contract loads the borrower’s stored scaled debt and the effective reserve (indexes accrued to now). Underlying debt = scaled debt × borrow index. So the returned debt is the current amount owed in lending units, including accrued interest. Response includes both scaled and underlying. Held collateral is always listed in `collateral` (`satisfiable` is false on holdings omitted from borrow-side USD). Missing, stale, or zero stored collateral prices are omitted from `collateral_value_usd`, `loan_to_value`, and `health` (same as Borrow) and those denoms are named in `unpriceable_collateral`. `liquidation_ltv` and `liquidation_health` use last-known prices still within `max_liquidation_staleness_seconds` (same as Liquidate); denoms Liquidate will not seize are named in `liquidation_unpriceable_collateral`. A stale **lending** denom still fails the query.
...
**Otherwise:** The contract fetches oracle prices for the lending denom and the requested collateral assets (and, if a borrower is given, their existing collateral denoms). The lending denom must be present and fresh. Missing, stale, or zero collateral prices are dropped (valued at $0) so a dead feed does not fail the query. It converts the new loan amount into a “value” (same units as prices). If a borrower is provided, it loads their current debt (effective reserve) and **existing collateral value** (same haircuts/oracle; unpriceable held assets contribute $0). **Total debt value** = existing debt value + new loan value. **Required total collateral value** (after haircuts) = total debt value ÷ margin rate. **Additional collateral value needed** = required total − existing collateral value (capped at zero).

**Per-asset minimums:** For each requested collateral asset, the minimum amount is the exact least units whose truncated haircutted `value_usd` covers **additional value needed** (or the full required value when no borrower). So when a borrower is set, the amounts are “how much **more** of each asset to add.”

**Response:** **required_collateral_value_usd**, **additional_collateral_value_usd** (use for “you need $X more” or to combine assets), **required**: (asset_id, min_amount, satisfiable) per requested asset, and **unpriceable_collateral**: held denoms omitted from the existing-collateral credit (same shape as GetBorrowerPosition; they may not appear in `required`). `satisfiable` is false when a positive requirement could not be quoted; the flag is deliberately coarse (down feed vs zero haircut vs unrepresentable units). Amount 0 with `satisfiable` true means need none. A UI can show total required, additional needed, and per-asset minimums.
