...
The liquidator also specifies *which* collateral to seize (denom and amount per asset). Assets with a stored **non-zero** price (fresh, or stale within `max_liquidation_staleness_seconds`) may be seized at that price. Assets with **no stored price**, a **zero** stored price, or last-known **beyond the bound** cannot. The **market value** of seized collateral (`display_price_usd × amount / 10^precision`; no haircut) must lie in the band: **≥ repay value** and **≤ repay value × liquidation_bonus**, except a **full close** waives the 100% floor. The liquidator therefore cannot take more than the bonus cap; they may pay more than the seized collateral is worth when emptying a dust position. Unpriceable balances stay on the borrower until a usable feed exists, and a mixed priced/unpriceable bag cannot full-close because the unpriceable portion cannot be seized.

The contract simulates collateral **after** the seizure before applying the band, so it knows whether the map would be empty. **Normal case:** some collateral remains; usual partial repay and debt update; the 100% floor still applies. **Bad-debt case:** no collateral would remain but debt would not be fully cleared—the borrower’s scaled debt is cleared, **total_scaled_borrow** drops by repay plus write-off, and reserve updates follow **`bad_debt_loss_allocation`** (deferred vs immediate; see Section 3). Cancelling all scaled debt against a remainder worth $0 — whether the map empties or an unpriceable unit is left behind — requires the ceiled payoff; sending only the floored debt is rejected so a one-unit rounding residue is not booked as bad debt. Response attributes include **`bad_debt_underlying`**, **`deficit_underlying`**, and **`bad_debt_loss_allocation`**. Collateral totals and borrower map are saved, seized coins go to the liquidator, and rate metadata is attached.
