...
| `liquidation_access` | **LiquidatorOnly** (default) or **Permissionless**. Who may call **Liquidate**. Set at instantiate or **UpdateContractConfig**. Permissionless requires last-known ≤ 1 hour, **immediate** haircut, and `deficit_underlying == 0`. Unpriceable collateral that is **load-bearing** (counting last-known of dropped feeds would make the position not liquidatable, or a dropped feed has no stored quote) still requires the owner. Dust of a dead feed does not. Paused still blocks Liquidate for everyone. |
