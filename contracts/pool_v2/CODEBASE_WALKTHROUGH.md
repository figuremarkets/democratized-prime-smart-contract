...
There is **no precomputed minimum repay**. The liquidator sends lending denom in **funds** (one coin, any non-zero amount); actual repay = min(sent, debt), and excess is refunded. There is no `amount` field in the message (same pattern as Repay). How much is *enough* is decided after the fact: the contract simulates the post-seizure collateral map and post-repay debt, then requires the resulting position to be **Healthy** (LTV ≤ **margin_rate**). A **full close**, a **full repayment**, and a post-state whose remaining collateral carries no USD value are exempt.
...
The contract simulates collateral **after** the seizure before applying the band, so it knows whether the map would be empty. **Normal case:** some collateral remains; usual partial repay and debt update; the 100% floor still applies. **Bad-debt case:** no collateral would remain but debt would not be fully cleared—the borrower’s scaled debt is cleared, **total_scaled_borrow** drops by repay plus write-off, and reserve updates follow **`bad_debt_loss_allocation`** (deferred vs immediate; see Section 3). Response attributes include **`bad_debt_underlying`**, **`deficit_underlying`**, and **`bad_debt_loss_allocation`**. Collateral totals and borrower map are saved, seized coins go to the liquidator, and rate metadata is attached.
