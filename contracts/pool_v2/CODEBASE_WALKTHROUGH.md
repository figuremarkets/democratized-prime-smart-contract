...
- **Scaling helpers** (Decimal256 with 18-decimal atomics ↔ u128):
  ...
  - **`scaled_to_underlying_borrow_ceil(scaled, borrow_index)`** – Ceil. Full close on Repay and Liquidate only.
...
The contract simulates collateral **after** the seizure before applying the band, so it knows whether the map would be empty. **Normal case:** some collateral remains; usual partial repay and debt update; the 100% floor still applies. **Bad-debt case:** no collateral would remain but debt would not be fully cleared—the borrower’s scaled debt is cleared, **total_scaled_borrow** drops by repay plus write-off, and reserve updates follow **`bad_debt_loss_allocation`** (deferred vs immediate; see Section 3). A full seizure that sends the floored debt but not the ceiled close is rejected so that one-unit residue is not booked as bad debt. Response attributes include **`bad_debt_underlying`**, **`deficit_underlying`**, and **`bad_debt_loss_allocation`**. Collateral totals and borrower map are saved, seized coins go to the liquidator, and rate metadata is attached.
