...
The user sends one or more coins that must be supported collateral types **and** have a **fresh oracle price** (present and not stale). The contract checks the borrower attribute and that the number of distinct collateral types (existing plus new) does not exceed the limit. It adds the received amounts to the borrower’s collateral balances and to the running totals per asset. The coins remain in the contract (they are the collateral). A supported asset with no live feed cannot be deposited, so a borrower cannot introduce an unpriceable denom into their map after borrowing.
...
Contract owner only. The contract accrues interest and loads the borrower's debt and collateral. Liquidation uses **last-known** oracle prices even when stale (so a paused feed does not freeze liquidations). The **lending denom** must still have a **stored** price. Collateral with **no stored price** is valued at **$0** for LTV and min-repay and **cannot be seized**. If every held collateral asset has no stored price, Liquidate fails. Borrow and RemoveCollateral still require **fresh** prices for every held asset. The borrower must be *liquidatable*: **LTV ≥ liquidation_rate** (using those valuations).
...
The contract owner also specifies *which* collateral to seize (denom and amount per asset). Assets with a stored price (fresh or stale) may be seized at that price. Assets with **no stored price** cannot. The **market value** of seized collateral (`display_price_usd × amount / 10^precision`; no haircut) must lie in the band: **≥ repay value** and **≤ repay value × liquidation_bonus**. So the liquidator receives collateral worth at least what they repaid, but not more than the bonus cap (profit is capped as intended). Unpriceable (no-price) balances stay on the borrower until a feed exists.
