...
| `deficit_underlying` | **Shortfall** in lending base units after a **bad-debt** close (Liquidate that exhausts collateral, or **WriteOff**): the borrower has no collateral left, but a slice of debt could not be repaid from what was seized. It is **not** scaled borrow. It does **not** earn borrower interest. Older deployments deserialize it as **0** (`#[serde(default)]`). |
...
| `max_liquidation_staleness_seconds` | Outer bound on last-known oracle prices for liquidation (default **24 hours**, hard cap **7 days**). Expired prices older than this are unpriceable (valued at $0, not seizable). `0` disables last-known. Borrow/RemoveCollateral still give no credit for collateral past the *oracle* freshness window (seconds); this bound applies only to Liquidate. Do not enable permissionless liquidation against the 7-day cap. |
...
| `bad_debt_loss_allocation` | **BadDebtLossAllocation** (default **deferred_to_deficit**): on bad-debt liquidation or **WriteOff**, either book **`deficit_underlying`** (then **EliminateDeficit** / **SocializeDeficit**) or apply an immediate pro-rata **`liquidity_index`** haircut (**immediate_liquidity_index_haircut**). Set at **instantiate** or **UpdateContractConfig** (allocation may change only when **`deficit_underlying`** is zero). |
| `operational_state` | **Active** / **Frozen** / **Paused**. **Frozen:** blocks new **Lend** and **Borrow** only. **Paused:** strictest—blocks user value movement (**Liquidate**, **WriteOff**, **Repay**, **Receive**, **Withdraw**, deficit clears, collateral moves, **WithdrawReserve**); only a small set of owner config executes (see **`contract.rs`**). Set via **SetOperationalState**. |
...
Used for borrow health checks and liquidations (LTV vs margin_rate / liquidation_rate). Health is implemented: get_borrower_health, validate_borrower_is_healthy; Borrow and RemoveCollateral enforce healthy LTV; Liquidate requires LTV ≥ liquidation_rate. **WriteOff** closes leftover debt when remaining collateral is worth less than one lending unit. What happens when a borrower's collateral no longer covers their debt (LTV too high) is described in **Section 5 (When collateral doesn't cover debt)**.
...
- **Liquidate** – **Contract owner only.** Target borrower must be liquidatable (LTV ≥ liquidation_rate). The liquidator sends one lending coin; repay is **min(sent, debt)** and must meet the computed minimum. They also choose **collateral_to_seize**; its **market** value must sit between 100% and the liquidation bonus multiple of the repay value. Seized collateral goes to the liquidator; debt and collateral are reduced on chain. **Bad debt path:** if the seizure would leave **no collateral** but **debt still on the books**, the contract zeroes that borrower’s debt and trims **total_scaled_borrow** correctly. Depending on **`bad_debt_loss_allocation`**, it either books **`deficit_underlying`** (**deferred**) or applies an immediate **`liquidity_index`** haircut (**immediate**). Attributes **`bad_debt_underlying`**, **`deficit_underlying`**, and **`bad_debt_loss_allocation`** describe the path. If remaining collateral is worth less than one lending unit, use **WriteOff** instead. **Blocked when Paused.**
- **WriteOff** – **Contract owner only.** Closes leftover debt when **priced** collateral **market** value (`display_price_usd × amount / 10^precision`) is **less than one lending base unit**. Unpriceable / delisted feeds contribute $0 to that bar so the position can still be written off, but those balances are **not seized** — they stay on the borrower (who can **RemoveCollateral** once debt is zero). Uses last-known prices within **`max_liquidation_staleness_seconds`** (same as Liquidate). Seizes only priced dust (sent to the owner). Optional lending funds are applied as repay first; residual scaled debt uses the same **`bad_debt_loss_allocation`** path as a bad-debt liquidation. Under **immediate** haircut, a residual that would floor `liquidity_index` to zero reverts. Emits **`unpriceable_json`**. **Paused:** blocked. **Frozen:** allowed.
...
- **SetOperationalState** – Contract owner only; no funds. Switches **Active**, **Frozen**, or **Paused**. **Paused** is the strictest: only a handful of owner config messages run; all user value movement (including **Liquidate**, **WriteOff**, **EliminateDeficit**, and **SocializeDeficit**) is off until you return to Active or Frozen.
...
- **LTV ≥ liquidation_rate:** **Liquidatable**. **Liquidate** pays down debt and pulls collateral within the value band. If seizure uses up all collateral but **debt remains**, leftover scaled debt is cleared; supplier impact follows **`bad_debt_loss_allocation`**: **`deficit_underlying`** (**deferred**) or an immediate **`liquidity_index`** haircut (**immediate**). A **deferred** deficit can be reduced later with **EliminateDeficit** (**`bank`** from any payer or **`accrued_reserve`** by the contract owner) or **SocializeDeficit** (pro-rata haircut).
- **Remaining priced collateral worth less than one lending unit** (unpriceable feeds count as $0 toward the bar, but are **not seized**): close the debt with owner **WriteOff**, which seizes only priced dust and books residual debt the same way. Unpriceable balances stay on the borrower.

**Liquidate** (`liquidate.rs`)
Contract owner only. The contract accrues interest and loads the borrower's debt and collateral. Liquidation uses **last-known** oracle prices when stale but still within **`max_liquidation_staleness_seconds`** of expiration (default **24 hours**, hard cap **7 days**; so a paused feed does not freeze liquidations). The **lending denom** must have a stored price within that bound. Collateral with **no stored price**, or last-known **older than the bound**, is valued at **$0** for LTV and min-repay and **cannot be seized**. If every held collateral asset is unpriceable, Liquidate fails. Borrow and RemoveCollateral give **no credit** for missing or stale collateral (TreatAsWorthless) as soon as the oracle marks the feed stale (typically tens of seconds) and still require a **fresh** lending-denom price — a much tighter window than liquidation last-known. The borrower must be *liquidatable*: **LTV ≥ liquidation_rate** (using those valuations). If remaining collateral is worth less than one lending unit, use **WriteOff**.

The contract computes the **minimum repay**: the repayment (in lending units) that would bring LTV back to exactly the margin rate, assuming we seize collateral whose value is between 100% and the liquidation-bonus rate (e.g. 102%) of that repay. This minimum is capped by the borrower's full debt and by total collateral value (so partial liquidation is possible). The caller sends lending denom in **funds** (one coin); actual repay = min(sent, debt), and sent must be ≥ minimum. Excess is refunded. There is no `amount` field in the message (same pattern as Repay).

The contract owner also specifies *which* collateral to seize (denom and amount per asset). Assets with a stored price (fresh, or stale within `max_liquidation_staleness_seconds`) may be seized at that price. Assets with **no stored price** or last-known **beyond the bound** cannot. The **market value** of seized collateral (`display_price_usd × amount / 10^precision`; no haircut) must lie in the band: **≥ repay value** and **≤ repay value × liquidation_bonus**. So the liquidator receives collateral worth at least what they repaid, but not more than the bonus cap (profit is capped as intended). Unpriceable balances stay on the borrower until a usable feed exists (or the borrower removes them after **WriteOff** zeros the debt).
...
**WriteOff** (`write_off.rs`)
Contract owner only. Use when **priced** collateral **market** value (`display_price_usd × amount / 10^precision`, no haircut) is below one lending base unit. Unpriceable feeds count as $0 toward that bar (so a delisted-feed position can be written off) but are **not seized**. Same last-known price path as Liquidate. Seizes priced dust only (sent to the owner), leaves unpriceable balances on the borrower, optionally applies attached lending funds as repay, and books residual scaled debt the same way as Liquidate’s bad-debt path. Immediate haircut reverts if the loss would floor `liquidity_index` to zero. Positions whose priced collateral is still worth at least one lending unit use Liquidate. Response includes **`unpriceable_json`**.
