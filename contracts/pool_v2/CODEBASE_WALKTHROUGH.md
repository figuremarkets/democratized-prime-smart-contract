...
| `max_liquidation_staleness_seconds` | Outer bound on last-known oracle prices for liquidation (default 7 days). Expired prices older than this are unpriceable (valued at $0, not seizable). `0` disables last-known. |
...
Contract owner only. The contract accrues interest and loads the borrower's debt and collateral. Liquidation uses **last-known** oracle prices when stale but still within **`max_liquidation_staleness_seconds`** of expiration (default 7 days; so a paused feed does not freeze liquidations). The **lending denom** must have a stored price within that bound. Collateral with **no stored price**, or last-known **older than the bound**, is valued at **$0** for LTV and min-repay and **cannot be seized**. If every held collateral asset is unpriceable, Liquidate fails. Borrow and RemoveCollateral give **no credit** for missing or stale collateral (TreatAsWorthless) but still require a **fresh** lending-denom price. The borrower must be *liquidatable*: **LTV ≥ liquidation_rate** (using those valuations).
