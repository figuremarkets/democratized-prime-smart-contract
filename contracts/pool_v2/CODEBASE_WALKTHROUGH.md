...
There is **no precomputed minimum repay**. The liquidator sends lending denom in **funds** (one coin, any non-zero amount); actual repay = min(sent, **ceil** debt), and excess is refunded. There is no `amount` field in the message (same pattern as Repay). How much is *enough* is decided after the fact: the contract simulates the post-seizure collateral map and post-repay debt, then requires the resulting position to be **Healthy** (LTV ≤ **margin_rate**). A **full close**, a **full repayment**, and a post-state whose remaining collateral carries no USD value are exempt.
