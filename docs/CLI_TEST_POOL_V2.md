...
To test liquidation you would: (a) ensure the borrower has debt and collateral; (b) **reprice** the collateral down in the price oracle so LTV ≥ 90% (e.g. `update_asset_prices` with a lower `usd` for `$COLLATERAL_DENOM`); (c) **fund admin** with YLDS (e.g. mint to admin or transfer from lender); (d) call **liquidate** with `amount` and `collateral_to_seize` matching the contract's rules (seize value within 100–102% of repay, and the repay plus seizure must leave the borrower's LTV at or below `margin_rate` — a seizure that empties the collateral map is exempt). Template (adjust amounts and only run when the position is actually liquidatable and admin has the YLDS):
